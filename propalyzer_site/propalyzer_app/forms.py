...
class PropertyForm(forms.Form):
    ...
    zestimate = forms.IntegerField(
        label="Zestimate (Current Value)", help_text="Pulled from Zillow.com"
    )
    down_payment_percentage = forms.FloatField(label="Down Payment %", max_value=100)
    interest_rate = forms.FloatField(label="Mortgage Interest Rate %", max_value=99)
    ...
    initial_improvements = forms.IntegerField(
        ...
        help_text="Enter any costs to renovate property",
        required=False,
    )
    ...
    taxes = forms.IntegerField(
        label="Taxes",
        help_text="Pulls latest tax assessment from Zillow's Public Records API. If not found, defaults to $2000. If modifying, enter annual amount",
    )
    utilities = forms.IntegerField(
        label="Utility Costs",
        help_text="If you will pay utility costs instead of the tenant -- enter monthly amount",
        required=False,
    )
    ...
    prop_management_fee = forms.IntegerField(
        ...
        help_text="Enter $ amount. Defaults to 9% of rent amount",
    )
    ...
    county = forms.CharField(
        ...
        max_length=150,
        required=False,
        help_text="Pulled from Zillow.com",
    )
